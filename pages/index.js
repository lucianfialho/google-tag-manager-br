...
export default function Home({feed}) {
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>GTMBR - HUB de produção e curadoria de conteúdo voltada para a comunidade de Google Tag Manager</title>
          ...
          {feed.items.map((post, i) => {
            return (
              <div key={i} className={styles.grid}>
                <a href={ post.guid } className={styles.card}>
                  <h2>{post.title}</h2>
                  <Moment fromNow>{post.isoDate}</Moment>
                </a>
              </div>
            )
          })}
